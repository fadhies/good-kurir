...
export default function AdminOrders() {
  ...
  const orderDay = useMemo(() => {
    const m = {};
    for (const o of orders) m[o.id] = makassarDateKey(new Date(o.created_date));
    return m;
  }, [orders]);

  const dateOptions = useMemo(() => {
    const s = new Set();
    for (const o of orders) s.add(orderDay[o.id]);
    s.add(makassarToday());
  ...
  const filtered = useMemo(() => {
    ...
    filter((o) => date === "all" ? true : orderDay[o.id] === date).
    filter((o) => {
  ...
  }, [orders, status, search, date, orderDay]);
                ...
                <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-primary selectable">#{String(o.id).slice(-6)}</span>
