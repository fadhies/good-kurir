// Konfigurasi tarif antar yang bisa diatur admin lewat menu Pengaturan Tarif.
// Disimpan sebagai satu record AppSetting { key: "tariffs", value: <JSON string> }.
...
export async function getTariffs(base44) {
  try {
    const rows = await base44.asServiceRole.entities.AppSetting.filter({ key: 'tariffs' }, '-created_date', 1);
    if (rows[0]?.value) {
      return { ...DEFAULT_TARIFFS, ...JSON.parse(rows[0].value) };
    }
  } catch {}
...
export function computeRemitPerTxn(tariffs) {
  return Number(tariffs?.driver_remit_per_txn ?? 0);
}